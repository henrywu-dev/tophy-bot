...
from tophy.strategy.indicators import calculate_atr, calculate_rsi, calculate_sma
...
class QuantumMomentumStrategy(BaseStrategy):
    ...
    def populate_entry_signals(self) -> None:
        ...
        momentum_recovering = (df["rsi"] > self.rsi_extreme_low) & (
            df["rsi"].shift(1) < self.rsi_extreme_low
        )
        ...
        buy_strong = volatility_expansion & momentum_confirmation & trend_bullish & volume_wave

        # Moderate setup: Divergence + Volume confirmation + Trend
        buy_moderate = bullish_divergence & volume_wave & trend_bullish
        ...
        momentum_weakening = (df["rsi"] < self.rsi_extreme_high) & (
            df["rsi"].shift(1) > self.rsi_extreme_high
        )
        ...
        sell_strong = bearish_divergence & momentum_weakened & trend_bearish

        sell_moderate = trend_bearish & volume_exhaustion
    ...
    def populate_exit_signals(self) -> None:
        ...
        momentum_reversal = (df["momentum"] * df["momentum"].shift(1)) < 0
